...
<script>
var product_page = new Array();
var now_page = 0;
var count_page = <?php echo count($pages);?>;
var ue_array = new Array();
var file_html = new Array();
<?php $timestamp = time(); ?>
<?php if(count($pages)==0): ?>product_page[0]=new Array();
file_html[0]=new Array();
product_page[0]['content_num'] = 0;
product_page[0]['file_num'] = 0;
product_page[0]['file_num'][0] = 0;
ue_array[0] = new Array();
<?php else: ?>
<?php if(is_array($pages)): $pages_k = 0; $__LIST__ = $pages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$page): $mod = ($pages_k % 2 );++$pages_k;?>product_page[<?php echo ($pages_k-1); ?>] = new Array();
file_html[<?php echo ($pages_k-1); ?>] = new Array();
product_page[<?php echo ($pages_k-1); ?>]['content_num'] = <?php echo count($page['content']);?>;
product_page[<?php echo ($pages_k-1); ?>]['file_num'] = new Array();
<?php if(is_array($page["content"])): $content_k = 0; $__LIST__ = $page["content"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$content): $mod = ($content_k % 2 );++$content_k;?>product_page[<?php echo ($pages_k-1); ?>]['file_num'][<?php echo ($content_k-1); ?>] = <?php echo count($content['file']);?>;<?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; endif; ?>
</script>
</head>
<body class="no_index">
	<div class="top">
		<div class="w980">
			<div class="login">
				<?php if(session('?email')): ?><a title="个人中心" class="return_other" href="javascript:void(0)" rel="<?php echo U('member/home');?>"><?php echo session('email');?>&nbsp;<font id="message_tips" color="#1DB1AB">(0)</font></a>
				<span><a class="return_other" rel="<?php echo U('member/index');?>" href="javascript:void(0)">我的说明书</a></span>
    ...
    <div class="w980 main_table">
        ...
        <div class="product_left tips_p">
			<textarea name="name" placeholder="请输入说明书标题" id="name" class="product_title"><?php echo ($product["name"]); ?></textarea>
			<textarea name="description" id="description"  placeholder="请输入关键字，以便用户检索。多个关键字请用空格分隔,关键词中不能出现特殊字符" class="product_description" id="description"><?php echo ($product["description"]); ?></textarea>
			<div id="mousetips">点击“查看结果”，则生成消费者页面保存在后台、生成对应的链接和二维码，</div>
        </div>
		<script>
		...
		<div class="product_right">
			...
			<?php if(false): ?><div class="pruduct_images">
				<?php if($product['image'] != ''): ?><input type="hidden" id="image" name="image" value="<?php echo ($product["image"]); ?>"><img class="uploadify_img" width="100%" src="<?php echo ($product["image"]); ?>" /><a href="javascript:void(0)" id="image_delete"></a><?php endif; ?>
					<div id="pruduct_images" >点击上传图片</div>
				</div><?php endif; ?>
				<?php if(is_array($pages)): $pages_k = 0; $__LIST__ = $pages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$page): $mod = ($pages_k % 2 );++$pages_k;?><div id="page<?php echo ($pages_k-1); ?>" class="product_page_count" <?php if(($pages_k-1) != 0): ?>style="height:0px"<?php endif; ?>>
					<input name="pages[<?php echo ($pages_k-1); ?>][subject]" value="<?php echo ($page[subject]); ?>" class="product_page_title" placeholder="请输入本页标题，比如“安装方法”，“售后服务”等。" rel="<?php echo ($pages_k-1); ?>" />
					<script>
						$(function(){
							ue_array[<?php echo ($pages_k-1); ?>] = new Array();
						});
					</script>
					<?php if(is_array($page["content"])): $content_k = 0; $__LIST__ = $page["content"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$content): $mod = ($content_k % 2 );++$content_k;?><div class="pruduct_box" rel="<?php echo ($content_k-1); ?>">
						...
						<div class="pruduct_content_box">
							<textarea style="height:0px;border:none;" id="product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_description" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][description]"><?php echo ($content["description"]); ?></textarea>
						</div>
						<script>
						$(function(){
							ue_array[<?php echo ($pages_k-1); ?>][<?php echo ($content_k-1); ?>] = UE.getEditor('product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_description');
							ue_array[<?php echo ($pages_k-1); ?>][<?php echo ($content_k-1); ?>].addListener('ready',function(){
								//ue_array[<?php echo ($pages_k-1); ?>][<?php echo ($content_k-1); ?>].setContent('<?php echo ($content["description"]); ?>');
							});
						});
						</script>
						<div class="pruduct_flie_box">
							<div id="product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file_box" style="margin:20px 0">
							<?php if(is_array($content["file"])): $file_k = 0; $__LIST__ = $content["file"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$file): $mod = ($file_k % 2 );++$file_k; if($file['type'] == 'image'): ?><div class="product_page_img"><img class="uploadify_img" width="100%" src="<?php echo ($file["path"]); ?>"><a class="img_delete" href="javascript:void(0);"></a><input type="hidden" value="<?php echo ($file["path"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][path]"><input type="hidden" value="<?php echo ($file["width"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][width]"><input type="hidden" value="<?php echo ($file["height"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][height]"><input type="hidden" value="image" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][type]"></div>
							<?php elseif($file['type'] == 'video'): ?>
							<?php if($file['status'] == 1): $file['height'] = $file['height']?$file['height']:490 ?>
							<?php $file['width'] = $file['width'] ? $file['width']:600 ?>
							<?php $swf_height = 498 * $file['height'] /$file['width'] ?>
							<div class="product_page_img"><OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" height="<?php echo ($swf_height); ?>" width="498" id="polyvplayer<?php echo ($file["vid"]); ?>"><PARAM NAME=movie VALUE="http://player.polyv.net/videos/<?php echo ($file["vid"]); ?>.swf"><param name="allowscriptaccess" value="always"><param name="wmode" value="opaque"><param name="allowFullScreen" value="true" /><EMBED src="http://player.polyv.net/videos/<?php echo ($file["vid"]); ?>.swf" width="498" height="<?php echo ($swf_height); ?>" TYPE="application/x-shockwave-flash" allowscriptaccess="always" wmode="opaque" name="polyvplayer<?php echo ($file["vid"]); ?>" allowFullScreen="true" /></EMBED></OBJECT><a class="img_delete" href="javascript:void(0);"></a><input type="hidden" value="<?php echo ($file["vid"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][vid]"><input type="hidden" value="<?php echo ($file["height"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][height]"><input type="hidden" value="<?php echo ($file["width"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][width]"><input type="hidden" value="video" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][type]"><input type="hidden" value="<?php echo ($file["first_image"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][first_image]"><input type="hidden" value="<?php echo ($file["mp4"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][mp4]"></div>
							<?php elseif($file['status'] == 0): ?>
							<div class="product_page_img"><img class="uploadify_img" width="100%" src="<?php echo ($file['first_image']); ?>"><a class="img_delete" href="javascript:void(0);"></a><input type="hidden" value="<?php echo ($file["vid"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][vid]"><input type="hidden" value="<?php echo ($file["height"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][height]"><input type="hidden" value="<?php echo ($file["width"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][width]"><input type="hidden" value="video" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][type]"><input type="hidden" value="<?php echo ($file["first_image"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][first_image]"><input type="hidden" value="<?php echo ($file["mp4"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][mp4]"><div rel="'+jsonobj.data[0].vid+'" class="uploadify_img_status"><span>根据国家政策规定，您的视频正在被审核。建议您先进行其他编辑。20分钟左右后再查看结果将会自动生效。</span></div></div>
							<?php elseif($file['status'] == 2): ?>
							<div class="product_page_img"><img class="uploadify_img" width="100%" src="<?php echo ($file['first_image']); ?>"><a class="img_delete" href="javascript:void(0);"></a><input type="hidden" value="<?php echo ($file["vid"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][vid]"><input type="hidden" value="<?php echo ($file["height"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][height]"><input type="hidden" value="<?php echo ($file["width"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][width]"><input type="hidden" value="video" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][type]"><input type="hidden" value="<?php echo ($file["first_image"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][first_image]"><input type="hidden" value="<?php echo ($file["mp4"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][mp4]"><div rel="'+jsonobj.data[0].vid+'" class="uploadify_img_status"><span>根据国家政策规定，您的视频审核失败。</span></div></div>
							<?php elseif($file['status'] == 3): ?>
							<div class="product_page_img"><img class="uploadify_img" width="100%" src="<?php echo ($file['first_image']); ?>"><a class="img_delete" href="javascript:void(0);"></a><input type="hidden" value="<?php echo ($file["vid"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][vid]"><input type="hidden" value="<?php echo ($file["height"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][height]"><input type="hidden" value="<?php echo ($file["width"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][width]"><input type="hidden" value="video" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][type]"><input type="hidden" value="<?php echo ($file["first_image"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][first_image]"><input type="hidden" value="<?php echo ($file["mp4"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file][<?php echo ($file_k-1); ?>][mp4]"><div rel="'+jsonobj.data[0].vid+'" class="uploadify_img_status"><span>根据国家政策规定，您的视频存在违法信息，已被删除。</span></div></div><?php endif; endif; endforeach; endif; else: echo "" ;endif; ?>
							</div>
							<input id="product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file" type="file" multiple="true">
						</div>
						<script>
						$(function(){
							//初始化图片上传
							$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify({
								'buttonText': '&nbsp;',
								...
								'multi' : false,
								'removeTimeout' : 0,
								'queueID':'product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file_box',
								...
								'onSelect': function(file){
									...
									if(file.type == '.mp4' ||file.type == '.avi' ||file.type == '.mov' ||file.type == '.f4v' ||file.type == '.mpg' ||file.type == '.wav' ||file.type == '.flv' ||file.type == '.wmv' ||file.type == '.3gp' ||file.type == '.rmvb' ||file.type == '.mkv' ||file.type == '.asf' ||file.type == '.264' ||file.type == '.ts' ||file.type == '.mts' ||file.type == '.dat' ||file.type == '.vob' ||file.type == '.mp3'){
										if(file.size > <?php echo C('limit_video_size');?>){
											...
											$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify('cancel','*');
											return false;
										}
										$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify("settings","uploader",'http://v.polyv.net/uc/services/rest?method=uploadfile');
										$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify("settings",'formData',{
											'writetoken':'ql0zuWS75mvbgAy-w1NvhywuXqnj1vTL',
											'JSONRPC':'{"Filedata.filename": "'+file.name+'","tag": "标签", "description": "视频文档描述"}'
										});
										$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify("settings","onUploadSuccess",function(file, data, response){
											var num = product_page[<?php echo ($pages_k-1); ?>]['file_num'][<?php echo ($content_k-1); ?>]++;
											...
											if(jsonobj.error == 0){
												...
												var html = '<div class="product_page_img"><div class="uploadify_img_box"><img class="uploadify_img" width="100%" src="'+first_image+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][type]" value="video"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][first_image]" value="'+first_image+'"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][vid]" value="'+jsonobj.data[0].vid+'"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][width]" value="'+original_definition[0]+'"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][height]" value="'+original_definition[1]+'"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][mp4]" value="'+jsonobj.data[0].mp4+'"></div><div rel="'+jsonobj.data[0].vid+'" class="uploadify_img_status"><span>根据国家政策规定，您的视频正在被审核。建议您先进行其他编辑。20分钟左右后再查看结果将会自动生效。</span></div></div>';
												/*$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file-queue').append(html);*/
												$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file_box').append(html);
											}else{
											...
											$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify('disable',false);
											return true;
										});
										...
										$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify("settings","uploader",'<?php echo U("file/uploadify");?>');
										$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify("settings",'formData',{
										...
										$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify("settings","onUploadSuccess",function(file, data, response){
											...
											if(info.status == '0'){
												...
												var html = '<div class="product_page_img"><img class="uploadify_img" width="100%" src="'+info.data.path+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][type]" value="image"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][width]" value="'+info.data.width+'"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][height]" value="'+info.data.height+'"><input type="hidden" name="pages[<?php echo ($pages_k-1); ?>][content][<?php echo ($content_k-1); ?>][file]['+num+'][path]" value="'+info.data.path+'"></div>';
												$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file_box').append(html);
											}
											$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify('disable',false);
											return true;
									...
									$('#product_<?php echo ($pages_k-1); ?>_content_<?php echo ($content_k-1); ?>_file').uploadify('disable',true);
									return true;
								},
			...
			<div class="product_right_right">
				<?php if(is_array($pages)): $pages_k = 0; $__LIST__ = $pages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$page): $mod = ($pages_k % 2 );++$pages_k;?><div class="right_box product_page <?php if(($pages_k-1) == 0): ?>active<?php endif; ?>" id="product_page<?php echo ($pages_k-1); ?>" rel="<?php echo ($pages_k-1); ?>">
					<p class="page_code">第<?php echo ($pages_k); ?>页</p>
					<p class="page_title"><?php echo ($page["subject"]); ?></p>
					<div class="edit_box" style="display: none;"><a class="edit" rel="<?php echo ($pages_k-1); ?>" href="javascript:void(0)"></a><a class="delete" rel="<?php echo ($pages_k-1); ?>" href="javascript:void(0)"></a></div>
					<input type="hidden" class="page_sort" value="<?php echo ($page["sort_id"]); ?>" name="pages[<?php echo ($pages_k-1); ?>][sort_id]">
				</div><?php endforeach; endif; else: echo "" ;endif; ?>
	...
	<script type="text/javascript">
		$(function() {
			...
			$('.return_other').click(function(){
				...
				}else if(obj.attr('rel') != '<?php echo U('member/index');?>'){
					window.onbeforeunload = null;
			...
			$('.product_info').on('click','.pruduct_box .content_hide',function(){
				...
				if(ue_array[now_page][content_box_id].hasContents()){
					str = ue_array[now_page][content_box_id].getContentTxt().substring(0,30);
				}
				...
				$('#product_'+now_page+'_content_'+num+'_file').uploadify({
					...
					'onSelect': function(file){
						...
						if(file.type == '.mp4' ||file.type == '.avi' ||file.type == '.mov' ||file.type == '.f4v' ||file.type == '.mpg' ||file.type == '.wav' ||file.type == '.flv' ||file.type == '.wmv' ||file.type == '.3gp' ||file.type == '.rmvb' ||file.type == '.mkv' ||file.type == '.asf' ||file.type == '.264' ||file.type == '.ts' ||file.type == '.mts' ||file.type == '.dat' ||file.type == '.vob' ||file.type == '.mp3'){
							...
							$('#product_'+now_page_num+'_content_'+num+'_file').uploadify("settings","uploader",'http://v.polyv.net/uc/services/rest?method=uploadfile');
							...
							$('#product_'+now_page_num+'_content_'+num+'_file').uploadify("settings","onUploadSuccess",function(file, data, response){
								...
								var info = eval("("+data+")")
								if(info.status == '0'){
									layer.alert(info.info);
								}else{
									var html = '<div class="product_page_img"><img class="uploadify_img" width="100%" src="'+info.data.path+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages['+now_page_num+'][content]['+num+'][file]['+file_num+'][path]" value="'+info.data.path+'"><input type="hidden" name="pages['+now_page_num+'][content]['+num+'][file]['+file_num+'][width]" value="'+info.data.widtj+'"><input type="hidden" name="pages['+now_page_num+'][content]['+num+'][file]['+file_num+'][height]" value="'+info.data.height+'"><input type="hidden" name="pages['+now_page_num+'][content]['+num+'][file]['+file_num+'][type]" value="image"></div>';
									$('#product_'+now_page_num+'_content_'+num+'_file-queue').append(html);
								}
								$('#product_'+now_page_num+'_content_'+num+'_file').uploadify('disable',false);
								return true;
						...
						$('#product_'+now_page+'_content_'+num+'_file').uploadify('disable',true);
						return true;
					},
					'onSelectError':function(file){
						file.type = file.type.toLowerCase();
	...
	<div class="feedback_content" style="padding:10px;height:auto">
		...
		<a href="http://kefu6.kuaishang.cn/bs/im.htm?cas=13001___254766&fi=13061" style="display:block;margin:10px 0;" target=_blank><img src="__PUBLIC__/img/kefu1.jpg" border='0' /></a>
		<a href="http://kefu6.kuaishang.cn/bs/im.htm?cas=13003___383498&fi=13063" style="display:block;margin:10px 0 15px;" target=_blank><img src="__PUBLIC__/img/kefu2.jpg" border='0' /></a>
	</div>
...
<script>
	...
	<?php if(!session('?member_id')): ?><div class="tips">
			...
			<div class="tips_botton"><a  class="return_other" rel="<?php echo U('member/register');?>">立即注册</a></div>
			</div>
		</div>
		<script type="text/javascript">
			$(function() {
				var unloadPageTip = function(){
					return "您输入的内容可能不会被保存!";
				};
