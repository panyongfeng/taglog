...
<body class="no_index">
    <div class="top">
		<div class="w980">
			<div class="login">
				...
				<?php else: ?>
				<span><a class="return_login" href="<?php echo U('member/login');?>" >登录</a></span>
				<a class="return_register" href="<?php echo U('member/register');?>">注册</a><?php endif; ?>
			</div>
    ...
    <div class="naver">
         <div class="w980">
            ...
            <a href="javascript:void(0);" class="return_index" >返回根目录</a>
        </div>
    </div>
    <div class="w980 main_table">
        ...
        <div class="product_left tips_p">
			...
			<?php if(!session('?email')): ?><div class="usertips">
				<div class="tips_button"><a class="down" href="javascript:void(0)"></a><a class="up" href="javascript:void(0)"></a></div>
				<div id="usertips" class="usertips_massage">
				您正在使用试用版，请不要关闭浏览器，以免正在编辑的内容丢失。建议注册一个账户，这样我们就能在账户内帮您保存管理所有文档。注册过程中您不会丢失正在编辑的内容。
				</div>
				<div class="usertips_botton"><a class="return_register" href="<?php echo U('member/register');?>">立即注册</a></div>
				
			</div>
			<script>
				$(function(){
					setTimeout('no_register_tips()',5000);
					$('.tips_button .up').click(function(){
						$('.usertips').css({height:35});
	...
	<script type="text/javascript">
		...
		$(function() {
			$('#product_save').click(function(){
				...
				$.post(post_url,$("#product_form").serialize(),function(data){
					if($('#product_id').length <= 0 && data.status == 1){
						...
						$('#usertips').html('由于您尚未注册，请保存此文档的编辑链接，以便下次访问<a  class="copy_button" target="_blank" href="'+url+'">'+url+'</a>。否则关闭浏览器您将无法找回已创建说明书。我们建议您注册一个账户，这样您已创建和编辑的文档将会保存到您的账号内。');
					}
			...
			$('.return_index').click(function(){
				$.get('<?php echo U("member/islogin");?>',function(data){
					if(data.data == 1){
						...
						if(product_id){
							var title = '您需要先注册一个账号，才能为您创建根目录，现在就去注册么？';
			...
			$('.product_right_right').on('mouseenter','.product_page',function(){
				...
				$('#mousetips').html('点击此处编辑按钮可以在中间编辑区域显示该页的详细信息。<br />点击此处删除按钮可以删除该页，请小心操作。<br />直接拖拽可以改变页面的顺序哦~');
			});
			//同步页面标题
			...
			$('.product_info').on('click','.pruduct_box .content_delete',function(){
				var obj = $(this);
				/*
				if($(this).parent().siblings('.pruduct_box').length > 0){
					if(layer.confirm('本描述单元内的所有文字、视频、图片都会被删除，确定要删除吗？',function(){return 1;},function(){return 0;})){
						$(this).parent().remove();
					}
				}else{
					layer.alert('本描述单元为该页面内唯一的描述单元，禁止删除',-1);
				}
				*/
			...
			$('#add_product_page').mouseenter(function(){
				$('#mousetips').html('点击此处可以在说明书中新增一页。届时消费者向右滑动手机可以看到。');
			});
			<?php if(false): ?>$('.pruduct_images').mouseenter(function(){
				$('#mousetips').html('说明书的首页大图，将会显示在您的说明书的第一页的最上面。点击直接上传。<br />如果图片已将上传，点击删除按钮可以删除已上传的图片。<br /> 您也可以不上传图片，此时说明书将没有头图，而会显示您用文字编辑的说明书标题。');
			});<?php endif; ?>
			$('.product_info').on('mouseenter','.pruduct_box .product_page_img',function(){
				$('#mousetips').html('这里是您已经上传的视频（图片）。点击删除按钮可以直接删除。');
			});
			$('.product_info').on('mouseenter','.pruduct_box .uploadify',function(){
				$('#mousetips').html('点击此处可以上传说明书的图片，尤其可以上传视频哦！<br />建议视频不要过长，因为太长的视频消费者拖拽查找不方便。较长的视频建议可以分段，配合不同步骤的文字上传。这样比较方便消费者检索查找。');
			});
			$('#add_page_content').mouseenter(function(){
				$('#mousetips').html('点击这里可以展开一个新的描述区域，方便您描述下一步骤。');
			});
			$('.product_page_title').mouseenter(function(){
				$('#mousetips').html('请输入本页标题，比如“安装方法”，“售后服务”等。');
			});
			$('#description').mouseenter(function(){
				$('#mousetips').html('请输入关键词。用于您对您的产品进行准确分类，方便消费者检索。');
			});
			$('.product_info').on('mouseenter','.pruduct_box',function(){
				$('#mousetips').html('点击右上角的叉叉可以删除本段描述，请注意：本段内的文字和视频（图片）都会被删除哦，所以请小心操作。点击左上角的"^"可以收起本段。');
			});
			$('.product_info').on('mouseenter','.pruduct_box .pruduct_content_box',function(){
				$('#mousetips').html('您可以在此处添加一段文字描述。可以使安装过程的一个步骤、使用注意事项的某一点等等。');
			});
			$('#product_submit').mouseenter(function(){
				$('#mousetips').html('点击此处，立即在云端生成您的说明书与二维码，去看看效果吧~<br />编辑过程中也可以去看看，随时可以返回编辑。');
			});
			<?php if(false): ?>//删除图片
			...
			$('.product_info').on('click','.pruduct_box .uploadify_img_status button',function(){
				var obj = $(this)
				$(this).siblings('span').html('<img src="http://yhb360.qiniudn.com/images/loading.gif"/>');
				var vid = $(this).parent().attr('rel');
				$.get('<?php echo U('product/sendvediostatus','vid=');?>'+ vid ,function(data){
					if(data.status == 1){
						obj.siblings('span').html('已审核成功，您可以保存当前页面内容并点击“查看结果”按钮进行查看');
						var swf_hright = obj.parent().siblings('.uploadify_img_box').find('.uploadify_img').height();
						obj.parent().siblings('.uploadify_img_box').find('.uploadify_img').remove();
						obj.parent().siblings('.uploadify_img_box').prepend('<OBJECT classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" height="'+swf_hright+'" width="100%" id="polyvplayer'+vid+'"><PARAM NAME=movie VALUE="http://player.polyv.net/videos/'+vid+'.swf"><param name="allowscriptaccess" value="always"><param name="wmode" value="opaque"><param name="allowFullScreen" value="true" /><EMBED src="http://player.polyv.net/videos/'+vid+'.swf" width="100%" height="'+swf_hright+'" TYPE="application/x-shockwave-flash" allowscriptaccess="always" wmode="opaque" name="polyvplayer'+vid+'" allowFullScreen="true" /></EMBED></OBJECT>​');
						obj.remove();
					}else if(data.data == 2){
						obj.siblings('span').html('审核失败');
						obj.remove();
					}else if(data.data == 3){
						obj.siblings('span').html('审核失败');
						obj.remove();
					}else{
						obj.siblings('span').html('根据国家政策规定，您的视频正在被审核。你可以先进行其他编辑，20分钟左右后将会自动生效。');
						var swf_hright = obj.parent().siblings('.uploadify_img_box').find('.uploadify_img').height();
					}
				});
			});
			...
			$('#pruduct_images').uploadify({
				...
				'onUploadSuccess':function(file, data, response){
					...
					if(info.status == '0'){
						...
						$('.pruduct_images').append('<input type="hidden" id="image" name="image" value="'+info.data.path+'"><img class="uploadify_img" src="'+info.data.path+'" /><a href="javascript:void(0)" id="image_delete"></a>');
			...
			$('#add_page_content').click(function(){
				...
				$('#product_'+now_page+'_content_'+num+'_file').uploadify({
					'buttonText': '点击添加图片或视频',
					'fileTypeDesc' : '图片或视频文件',
					'fileTypeExts' : '*.avi;*.f4v;*.mpg;*.mp4;*.flv;*.wmv;*.mov;*.3gp;*.rmvb;*.mkv;*.asf;*.264;*.ts;*.mts;*.dat;*.vob;*.mp3;*.wav;*.jpg;*.jpeg;*.gif;*.png',//文件类型过滤
					'swf': './js/uploadify.swf',
					'multi' : false,
					'removeTimeout' : 0,
					'uploadLimit':1,
					...
					'onSelect': function(file){
						...
						if(file.type == '.mp4' ||file.type == '.avi' ||file.type == '.mov' ||file.type == '.f4v' ||file.type == '.mpg' ||file.type == '.wav' ||file.type == '.flv' ||file.type == '.wmv' ||file.type == '.3gp' ||file.type == '.rmvb' ||file.type == '.mkv' ||file.type == '.asf' ||file.type == '.264' ||file.type == '.ts' ||file.type == '.mts' ||file.type == '.dat' ||file.type == '.vob' ||file.type == '.mp3'){
							...
							$('#product_'+now_page+'_content_'+num+'_file').uploadify("settings","onUploadSuccess",function(file, data, response){
								var file_num = product_page[now_page]['file_num'][num]++;
								var jsonobj=eval('('+data+')');
								if(jsonobj.error == 0){
									var first_image = jsonobj.data[0].first_image.replace(/.jpg/, "_b.jpg");
									var html = '<div class="product_page_img"><div class="uploadify_img_box"><img class="uploadify_img" src="'+first_image+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][type]" value="video"><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][first_image]" value="'+first_image+'"><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][vid]" value="'+jsonobj.data[0].vid+'"><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][width]" value="'+jsonobj.data[0].playerwidth+'"><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][height]" value="'+jsonobj.data[0].playerheight+'"><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][mp4]" value="'+jsonobj.data[0].mp4+'"></div><div rel="'+jsonobj.data[0].vid+'" class="uploadify_img_status"><span>根据国家政策规定，您的视频正在被审核。你可以先进行其他编辑，20分钟左右后将会自动生效。</span><button type="button">更新状态</button></div></div>';
									$('#product_'+now_page+'_content_'+num+'_file-queue').append(html);
								}else{
							...
							if(file.size > 8192000){
								layer.alert('选择图片超过8M，请重新选择小于8M的图片！',-1);
								$('#product_'+now_page+'_content_'+num+'_file').uploadify("cancel");
							}
							$('#product_'+now_page+'_content_'+num+'_file').uploadify("settings","uploader",'<?php echo U("file/uploadify");?>');
							$('#product_'+now_page+'_content_'+num+'_file').uploadify("settings",'formData',{
								'timestamp':'<?php echo ($timestamp); ?>',
								'token':'<?php echo md5("instructions" . $timestamp);?>'
							});
							$('#product_'+now_page+'_content_'+num+'_file').uploadify("settings","onUploadSuccess",function(file, data, response){
								...
								if(info.status == '0'){
									...
									var html = '<div class="product_page_img"><img class="uploadify_img" src="'+info.data.path+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][type]" value="image"><input type="hidden" name="pages['+now_page+'][content]['+num+'][file]['+file_num+'][path]" value="'+info.data.path+'"></div>';
									$('#product_'+now_page+'_content_'+num+'_file-queue').append(html);
								}
								$('#product_'+now_page+'_content_'+num+'_file').uploadify('disable',false);
							});
			...
			$(".product_right_right").sortable({
				...
				stop:function(event,ui){
					...
					$('#page'+now_page).show();
					var i = 1;
					while($('#page'+now_page).children('.pruduct_box').length >= i){
						alert(1);
						$('#product_'+now_page+'_content_'+i+'_file').uploadify('upload');
						i++;
					}
					$.each($(".product_right_right .product_page"), function(i, item){
						var sort = i+1;
			...
			$('#product_'+now_page+'_content_0_file').uploadify({
				...
				'onSelect': function(file){
					...
					if(file.type == '.mp4' ||file.type == '.avi' ||file.type == '.mov' ||file.type == '.f4v' ||file.type == '.mpg' ||file.type == '.wav' ||file.type == '.flv' ||file.type == '.wmv' ||file.type == '.3gp' ||file.type == '.rmvb' ||file.type == '.mkv' ||file.type == '.asf' ||file.type == '.264' ||file.type == '.ts' ||file.type == '.mts' ||file.type == '.dat' ||file.type == '.vob' ||file.type == '.mp3'){
						...
						$('#product_'+now_page+'_content_0_file').uploadify("settings","uploader",'http://v.polyv.net/uc/services/rest?method=uploadfile');
						$('#product_'+now_page+'_content_0_file').uploadify("settings",'formData',{
							'writetoken':'ql0zuWS75mvbgAy-w1NvhywuXqnj1vTL',
							'JSONRPC':'{"Filedata.filename": "'+file.name+'","tag": "标签", "description": "视频文档描述"}'
						});
						$('#product_'+now_page+'_content_0_file').uploadify("settings","onUploadSuccess",function(file, data, response){
							var num = product_page[now_page]['file_num'][0]++;
							var jsonobj=eval('('+data+')');
							if(jsonobj.error == 0){
								var first_image = jsonobj.data[0].first_image.replace(/.jpg/, "_b.jpg");
								var html = '<div class="product_page_img"><div class="uploadify_img_box"><img class="uploadify_img" src="'+first_image+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][type]" value="video"><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][first_image]" value="'+first_image+'"><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][vid]" value="'+jsonobj.data[0].vid+'"><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][width]" value="'+jsonobj.data[0].playerwidth+'"><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][height]" value="'+jsonobj.data[0].playerheight+'"><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][mp4]" value="'+jsonobj.data[0].mp4+'"></div><div rel="'+jsonobj.data[0].vid+'" class="uploadify_img_status"><span>根据国家政策规定，您的视频正在被审核。你可以先进行其他编辑，20分钟左右后将会自动生效。</span><button type="button">更新状态</button></div></div>';
								$('#product_'+now_page+'_content_0_file-queue').append(html);
							}else{
								switch(jsonobj.error){
								case 1:
									layer.alert('找不到writetoken关联的user',-1);
						...
						$('#product_'+now_page+'_content_0_file').uploadify('disable',true);
					}else if(file.type == '.jpg' ||file.type == '.jpeg' ||file.type == '.gif' ||file.type == '.png'){
						if(file.size > 8192000){
							layer.alert('选择图片超过8M，请重新选择小于8M的图片！',-1);
							$('#product_'+now_page+'_content_0_file').uploadify("cancel");
						}
						$('#product_'+now_page+'_content_0_file').uploadify("settings","uploader",'<?php echo U("file/uploadify");?>');
						$('#product_'+now_page+'_content_0_file').uploadify("settings",'formData',{
							'timestamp':'<?php echo ($timestamp); ?>',
							'token':'<?php echo md5("instructions" . $timestamp);?>'
						});
						$('#product_'+now_page+'_content_0_file').uploadify("settings","onUploadSuccess",function(file, data, response){
							...
							if(info.status == '0'){
								...
								var html = '<div class="product_page_img"><img class="uploadify_img" src="'+info.data.path+'" /><a href="javascript:void(0);" class="img_delete"></a><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][type]" value="image"><input type="hidden" name="pages['+now_page+'][content][0][file]['+num+'][path]" value="'+info.data.path+'"></div>';
								$('#product_'+now_page+'_content_0_file-queue').append(html);
							}
							$('#product_'+now_page+'_content_0_file').uploadify('disable',false);
						});
	...
	<div class="feedback_content">
		...
		<div class="feedback_content_box">
			<textarea id="feedback_content" placeholder="在这里留下您的意见"></textarea>
			<div class="feedback_email_box">
				<input type="submit" id="sub_feedback" value="提交反馈"/>
				<input type="text" id="feedback_email" placeholder="请输入您的邮箱"/>
			</div>
		</div>
	</div>
...
<script>
	...
	$(function(){
		...
		$(".feedback").mouseleave(function(){
			if($("#feedback_content").val() == '' && $("#feedback_email").val() == ''){
				$(".feedback").width(46);
				$(".feedback_content").hide();
				$(".feedback_button").show();
			}
	...
	<script type="text/javascript">
		...
		<?php if(!session('?member_id')): ?>$(function() {
				var unloadPageTip = function(){
					return "您可能尚未注册，将丢失已编辑内容；建议您先“取消”，然后点击右上角“保存按钮”。现在点击“确认”将强行离开!";
				};
				window.onbeforeunload = unloadPageTip;
			});
		<?php else: ?>
			$(function() {
				var unloadPageTip = function(){
					return "直接离开将丢失已编辑内容，建议您先“取消”，然后点击右上角“保存”按钮。现在点击“确认”将强行离开。";
				};
				window.onbeforeunload = unloadPageTip;
			});<?php endif; ?>
	</script>
